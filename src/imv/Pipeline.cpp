...
template<bool EXTRA_INDIRECTION>
size_t _filter_probe_imv_nobuf(size_t begin, size_t end, Database& db, runtime::Hashmap* hash_table, void** output_build, uint32_t*output_probe, uint64_t* pos_buff) {
  auto& li = db["lineitem"];
  auto l_quantity_col = li["l_quantity"].data<types::Numeric<12, 2>>();
#if PIPELINE_ORDERED
  auto l_orderkey = li["l_orderkey"].data<types::Integer>();
    ...
    switch (imv_state[k].stage) {
      ...
      case 2: {
        ...
        if (num == 0) {
          ...
          imv_state[k].stage = EXTRA_INDIRECTION ? 8 : 0; // introduce extra indirections
          v_prefetch(imv_state[k].v_bucket_addrs);
        } 
#endif
      }
      ...
      case 9:
        imv_state[k].stage = 10;
        break;

      case 10:
        imv_state[k].stage = 11;
        break;

      case 11:
        imv_state[k].stage = 12;
        break;
...
size_t filter_probe_imv_nobuf(size_t begin, size_t end, Database& db, runtime::Hashmap* hash_table, void** output_build, uint32_t*output_probe, uint64_t* pos_buff, bool extra_indirection) {
  if (extra_indirection) {
    return _filter_probe_imv_nobuf<true>(begin, end, db, hash_table, output_build, output_probe, pos_buff);
  } else {
    return _filter_probe_imv_nobuf<false>(begin, end, db, hash_table, output_build, output_probe, pos_buff);
  }
}
